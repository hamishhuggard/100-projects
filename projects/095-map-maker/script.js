function processUrl() {
    const inputField = document.getElementById("sheetUrl");
    const url = inputField.value;
    const messageElement = document.getElementById("message");

    const regex = /^https:\/\/docs\.google\.com\/spreadsheets\/d\/[a-zA-Z0-9-_]+\/edit\?usp=sharing$/;

    if (regex.test(url)) {
        const newUrl = url.replace("/edit?usp=sharing", "/export?format=csv&gid=1371825706&single=true&output=csv");
        messageElement.textContent = "URL is valid. Creating map...";
        createMap(newUrl);
    } else {
        messageElement.textContent = "Invalid Google Sheets URL. Please check and try again.";
    }
}

function createMap(csvUrl) {
    console.log("Creating map with URL:", csvUrl);
    // Add your map creation logic here
}
